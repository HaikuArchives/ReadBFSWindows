...
class Inode {
	public:
		...
		status_t ReadAttribute(const char *name, int32 type, off_t pos, uint8 *buffer, off_t *_length);
		...
		status_t ReadAt(off_t pos, uint8 *buffer, off_t *length);
...
class AttributeIterator {
	public:
		...
		status_t GetNext(char *name, size_t *length, uint32 *type, unsigned long long *id);
