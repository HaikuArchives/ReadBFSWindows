...
dump_super_block(const disk_super_block *superBlock)
{
	...
	Print("  name           = %s\n", superBlock->name);
	Print("  magic1         = %#08x (%s) %s\n", (int)superBlock->magic1, get_tupel(superBlock->magic1), ((superBlock->magic1 == SUPER_BLOCK_MAGIC1) || (superBlock->magic1 == BFS_SUPER_BLOCK_MAGIC1) ? "valid" : "INVALID"));
	...
	Print("  magic2         = %#08x (%s) %s\n", (int)superBlock->magic2, get_tupel(superBlock->magic2), ((superBlock->magic2 == (int)SUPER_BLOCK_MAGIC2) || (superBlock->magic1 == BFS_SUPER_BLOCK_MAGIC2) ? "valid" : "INVALID"));
	...
	Print("  log_end        = %Lu\n", superBlock->log_end);
	Print("  magic3         = %#08x (%s) %s\n", (int)superBlock->magic3, get_tupel(superBlock->magic3), ((superBlock->magic3 == SUPER_BLOCK_MAGIC3) || (superBlock->magic1 == BFS_SUPER_BLOCK_MAGIC3) ? "valid" : "INVALID"));
	...
	for (int32 i = 0;i < node->all_key_count;i++) {
		...
		if ((uint32)value < (uint32)node || (uint32)value > (uint32)node + header->node_size)
			...
			if (header->data_type == BPLUSTREE_STRING_TYPE)
				...
				Print("int64 = %Ld (0x%Lx)", *(int64_ *)&buffer, *(int64_ *)&buffer);
			else
				Print("???");
...
{
	if (argc < 2) {
		printf("usage: bfsinode <ptr-to-inode>\n");
		return 0;
	}
// TODO: get this right?
