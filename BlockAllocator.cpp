...
{
	printf("## BlockAllocator::Initialize\n");
...
//TODO: convert this to windows
	...
	// long WINAPI Thread()
	HANDLE id = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)BlockAllocator::_Initialize, 0, 0, 0);
	//int id=-1;
	//if (id < B_OK)
...
BlockAllocator::_Initialize(BlockAllocator *allocator)
{
	...
	printf("BlockAllocator::_Initialize called from thread 0x%p\n",GetCurrentThread());
	...
	for (int32 i = 0; i < numGroups; i++) {
		printf("BlockAllocator::_Initialize: read_pos group %i/%i\n",i,numGroups);
		if (read_pos(volume->Device(), /*volume->StartPos()+*/ (offset << blockShift), buffer,
...
BlockAllocator::CheckNextNode(check_control *control)
{
	...
	while (true) {
		...
		if (status == B_ENTRY_NOT_FOUND) {
			...
			strcpy(control->name, name/*, B_FILE_NAME_LENGTH*/);
			control->inode = id;
			control->errors = 0;
