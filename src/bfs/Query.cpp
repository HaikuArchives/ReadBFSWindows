...
class Term {
	public:
		...
		virtual status_t Match(Inode *inode,const char *attribute = NULL,int32 type = 0,
				const uint8 *key = NULL,off_t size = 0) = 0;
...
class Equation : public Term {
	public:
		...
		virtual status_t Match(Inode *inode, const char *attribute = NULL, int32 type = 0,
						const uint8 *key = NULL, off_t size = 0);
...
Equation::ConvertValue(type_code type)
{
	...
	switch (type) {
		...
		case B_UINT32_TYPE:
			...
			break;
		case B_INT64_TYPE:
			//TODO, this should be strtoll, but it's not used at all
			fValue.Int64 = strtol(string, &string, 0);
			fSize = sizeof(int64_);
			break;
		case B_UINT64_TYPE:
			//TODO, this should be strtoull, but it's not used at all
			fValue.Uint64 = strtoul(string, &string, 0);
			fSize = sizeof(uint64);
			break;
		case B_FLOAT_TYPE:
...
Equation::CompareTo(const uint8 *value, uint16 size)
{
	...
	if (fIsPattern) {
		...
		int64_ timeValue = *(int64_ *)value >> INODE_TIME_SHIFT;
		compare = compareKeys(fType, &timeValue, sizeof(int64_), &fValue.Int64, sizeof(int64_));
	} else
...
status_t
Equation::Match(Inode *inode, const char *attributeName, int32 type, const uint8 *key, off_t size)
{
...
status_t
Operator::Match(Inode *inode, const char *attribute, int32 type, const uint8 *key, off_t size)
{
	if (fOp == OP_AND) {
